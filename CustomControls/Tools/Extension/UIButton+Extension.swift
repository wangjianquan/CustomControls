...
extension UIButton {
    ...
    class func createImage(normal: String, seleImage: String) -> UIButton{
        ...
        btn.setImage(UIImage(named:normal), for: .selected)
        btn.setBackgroundImage(UIImage(named:seleImage), for: .normal)
        btn.setBackgroundImage(UIImage(named:seleImage), for: .selected)
        btn.sizeToFit()
        return btn
    }
  ...
  convenience  init(normalImg: String?, seleImg: String?)
  {
        ...
        if let sele = seleImg
        {
            ...
            setBackgroundImage(UIImage(named: sele), for: .highlighted)
        }
